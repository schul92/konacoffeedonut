...
const content: Record<Locale, BlogContent> = {
  en: {
    ...
    intro:
      "Hawaii is the only US state that grows coffee, and Kona — on the Big Island's western slope — is where the legendary stuff comes from. The challenge: most \"Kona blend\" coffee in Waikiki tourist shops contains as little as 10% actual Kona coffee. The other 90% can be cheap commodity beans from Brazil or Vietnam. If you want to taste what real Kona coffee actually tastes like, you need to know where to find Honolulu Coffee — and how to brew it right. This guide tells you exactly where to drink it in Waikiki and what to look for.",
    visitCTA: {
      headline: 'Authentic Honolulu Coffee on Kalākaua Avenue',
      body:
        "We brew with Honolulu Coffee sourced through Honolulu Coffee — no blends, no fillers. Pour-over, espresso, lattes, and whole beans to take home. Walking distance from Waikiki Beach. Open daily 7 AM to 9 PM.",
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Real Honolulu Coffee is rare in tourist-area shops',
        ],
      },
      {
        h2: 'Kona Blend vs. Single-Origin Kona',
        body:
          "Hawaii state law currently allows a coffee to be labeled \"Kona Blend\" with as little as 10% actual Kona beans. The remaining 90% can come from anywhere. If a shop is hesitant or vague about the percentage, it's probably a blend. The simplest move is to ask the barista what coffee they brew and where it comes from. At Kona Coffee Donut we serve Honolulu Coffee, a respected Hawaii roaster, so you always know what's in your cup.",
        pullout: {
          title: 'When in doubt, ask',
          body: '"Kona Blend" can mean as little as 10% Kona. Ask the barista exactly what they brew — we\'re happy to tell you we serve Honolulu Coffee.',
        },
      },
      {
        h2: 'How We Brew Kona Coffee',
        body:
          "At Kona Coffee Donut we brew Honolulu Coffee through several methods so you can taste the difference. Pour-over highlights the cleanest flavor and is our recommendation for first-timers — you taste exactly what the bean offers. Espresso pulls a more concentrated, syrupy version. Iced Americano is great on a hot Waikiki day and lets the floral notes shine through. Latte and cappuccino balance the coffee with steamed milk for a richer, smoother experience.",
        bullets: [
  ...
  ja: {
    hero: { title: 'ワイキキで本物のコナコーヒーを飲める場所', subtitle: '大手チェーンを超えて — カラカウア通りでホノルルコーヒーを。', date: '2026年5月更新', readTime: '読了6分', badge: 'コーヒーガイド' },
    intro: 'ハワイ島コナ地区で育った世界的に有名なコナコーヒー。観光地で売られている「コナブレンド」の多くは実際には10%しかコナ豆を含まないこともあります。ホノルルコーヒーを味わいたいなら、どこに行くべきかを知ることが重要です。',
    visitCTA: { headline: 'カラカウア通りでホノルルコーヒー', body: 'ホノルルコーヒー経由のホノルルコーヒーのみ使用。プアオーバー、エスプレッソ、ラテ、お持ち帰り豆も。毎日朝7時〜夜9時。', menuLabel: 'コーヒーメニュー', directionsLabel: '行き方を確認' },
    sections: [
      ...
      { h2: 'コナブレンドと単一産地コナの違い', body: 'ハワイ州法では「コナブレンド」は10%のコナ豆でも表示可能です。提供しているコーヒーの産地・銘柄を店員に確認するのが確実です。当店ではHonolulu Coffeeを提供しています。', pullout: { title: '迷ったら質問を', body: '「コナブレンド」は10%しかコナを含まない場合があります。当店ではHonolulu Coffeeを提供しているので、お気軽にお尋ねください。' } },
      { h2: '当店の抽出方法', body: '複数の方法で味わいの違いを楽しめます。', bullets: ['プアオーバー — 初めての方に', 'エスプレッソ — アフォガート向き', 'アイスアメリカーノ — 暑い日に', 'ラテ・カプチーノ — まろやか', 'コールドブリュー — 自然な甘み'] },
      { h2: 'お土産にも', body: '当店ではホノルルコーヒーの豆を袋売り。お土産に最適です。' },
    ],
    faq: [
      { q: 'ワイキキでホノルルコーヒーはどこで？', a: 'コナコーヒードーナツ（2142 カラカウア通り）。ホノルルコーヒー経由のホノルルコーヒーのみ使用。' },
      { q: 'ブレンドとの違いは？', a: 'ブレンドは10%しか本物のコナを含まない場合あり。ホノルルコーヒーははるかに高品質。' },
  ...
  ko: {
    ...
    intro: '하와이 빅아일랜드 코나 지역에서 자란 세계적인 코나커피. 관광지의 "코나 블렌드"는 실제로는 10%만 진짜 코나일 수 있습니다. 호놀룰루 커피를 마시려면 정확히 어디로 가야 하는지 알아야 합니다.',
    visitCTA: { headline: '칼라카우아 거리의 호놀룰루 커피', body: '호놀룰루 커피의 호놀룰루 커피만 사용. 푸어오버, 에스프레소, 라테, 원두 판매. 매일 7시–21시.', menuLabel: '커피 메뉴', directionsLabel: '길찾기' },
    sections: [
      { h2: '코나커피가 특별한 이유', body: '화산토양, 손 수확 등이 독특한 풍미를 만듭니다.', bullets: ['화산토양 재배', '손으로 수확', '부드럽고 산미 낮음', '세계적으로 비싼 커피', '관광지에서는 진품 구하기 어려움'] },
      { h2: '코나 블렌드 vs 단일 원산지 코나', body: '하와이 주법은 10%만으로도 "코나 블렌드" 라벨을 허용합니다. 매장에서 어떤 커피를 제공하는지 바리스타에게 직접 물어보는 것이 가장 확실합니다. 저희 매장은 Honolulu Coffee를 제공합니다.', pullout: { title: '궁금하면 물어보세요', body: '"코나 블렌드"는 코나가 10%만 들어있을 수 있습니다. 저희는 Honolulu Coffee를 제공합니다.' } },
      { h2: '당점의 추출 방식', body: '여러 방식으로 차이를 즐길 수 있습니다.', bullets: ['푸어오버 — 처음 분께 추천', '에스프레소 — 아포가토에 적합', '아이스 아메리카노 — 더운 날', '라테/카푸치노 — 부드러움', '콜드 브루 — 자연스런 단맛'] },
      { h2: '원두로 가져가기', body: '호놀룰루 커피 판매 — 하와이 기념품으로 최고.' },
    ],
    faq: [
      { q: '와이키키 어디에서 호놀룰루 커피?', a: '코나커피도넛(2142 칼라카우아 거리). 호놀룰루 커피 호놀룰루 커피 사용.' },
  ...
  zh: {
    ...
    visitCTA: { headline: '卡拉考阿大道的檀香山咖啡', body: '通过 Honolulu Coffee 进口的檀香山咖啡。手冲、浓缩、拿铁、咖啡豆销售。每天7点–21点。', menuLabel: '咖啡菜单', directionsLabel: '获取路线' },
    sections: [
      { h2: '科纳咖啡的特别之处', body: '火山土壤、人工采摘等条件造就独特风味。', bullets: ['火山土壤种植', '人工精选采摘', '顺滑低酸', '全球昂贵咖啡', '旅游区难找正宗'] },
      { h2: '科纳混合 vs 单一产地科纳', body: '夏威夷法律允许只含10%科纳豆即可标"科纳混合"。最稳妥的方式是直接询问咖啡师所用的咖啡是什么。我们门店提供 Honolulu Coffee。', pullout: { title: '不确定就直接问', body: '"科纳混合" 可能只含10%科纳。我们门店提供 Honolulu Coffee，欢迎询问。' } },
      { h2: '我们的萃取方式', body: '多种方式品鉴差异。', bullets: ['手冲 — 推荐初次体验', '浓缩 — 适合阿芙佳朵', '冰美式 — 炎热天气', '拿铁/卡布奇诺 — 顺滑', '冷萃 — 自然甜味'] },
      { h2: '带回家', body: '檀香山咖啡袋装出售 — 夏威夷最佳伴手礼之一。' },
    ],
    faq: [
      { q: '威基基哪里有檀香山咖啡？', a: 'Kona Coffee Donut（卡拉考阿大道2142号），使用 Honolulu Coffee 檀香山咖啡。' },
  ...
  es: {
    ...
    visitCTA: { headline: 'Honolulu Coffee en Kalākaua', body: 'Servimos Honolulu Coffee. Pour-over, espresso, lattes, y granos para llevar. 7 AM a 9 PM diariamente.', menuLabel: 'Ver Menú de Café', directionsLabel: 'Cómo llegar' },
    sections: [
      ...
      { h2: 'Honolulu Coffee vs Mezcla Kona', body: 'La ley permite etiquetar "Mezcla Kona" con tan solo 10% de Kona real. Pregunta al barista qué café se sirve. En nuestra cafetería servimos Honolulu Coffee.', pullout: { title: 'Consejo', body: 'Si no estás seguro del origen, pregunta directamente al barista.' } },
      ...
      { h2: 'Llévalo a Casa', body: 'Vendemos granos Honolulu Coffee — perfecto recuerdo de Hawái.' },
    ],
    faq: [
      { q: '¿Dónde encontrar Honolulu Coffee en Waikiki?', a: 'Kona Coffee Donut, 2142 Kalākaua Ave.' },
