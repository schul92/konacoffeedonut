            ...
            <tbody>
              {data.topPages.map((r, i) => (
                <tr key={i} className="border-b border-stone-50 last:border-0">
                  <td className="py-1.5 pr-2 text-stone-800 truncate max-w-[320px]">{r.path}</td>
                  <td className="text-right tabular-nums">{num(r.views)}</td>
                  <td className="text-right tabular-nums text-stone-500 w-16">{num(r.users)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-sm">
            <tbody>
              {data.channels.map((r, i) => (
                <tr key={i} className="border-b border-stone-50 last:border-0">
                  <td className="py-1.5 pr-2 text-stone-800">{r.channel}</td>
