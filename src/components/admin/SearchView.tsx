          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b border-stone-50 last:border-0">
                <td className="py-1.5 pr-2 text-stone-800 truncate max-w-[260px]">{r.key}</td>
                <td className="text-right tabular-nums">{num(r.clicks)}</td>
                <td className="text-right tabular-nums text-stone-500">{num(r.impressions)}</td>
                <td className="text-right tabular-nums text-stone-500">{pct(r.ctr)}</td>
                <td className="text-right tabular-nums text-stone-500">{r.position.toFixed(1)}</td>
              </tr>
            ))}
          </tbody>
        </table>
